...
async def synthesize_code(spec: str) -> dict[str, str]:
    ...
    return _parse_files(msg.content[0].text)
...
async def _get_app_token() -> str:
    ...
    if not (app_id and key_path and inst_id):
        raise RuntimeError(
            "GitHub App not configured — set GITHUB_APP_ID, "
            "GITHUB_APP_PRIVATE_KEY_PATH, GITHUB_APP_INSTALLATION_ID in .env"
        )
    # Relative paths in .env are relative to the project root (parent of issuer/).
